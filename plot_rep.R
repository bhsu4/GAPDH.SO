...
plot_together <- function(xs, params, params2, params3, params4){
  ...
  for(k in 2:nrow(params)){
    lines(x=xs, y=l4(xs, b=params$b[k], c=params$c[k],
                     d=params$d[k], e=params$e[k]), col=k)
    lines(x=xs, y=l5(xs, b=params2$b[k], c=params2$c[k],
                     d=params2$d[k], e=params2$e[k], f=params2$f[k]), col=k)
    lines(x=xs, y=b4(xs, b=params3$b[k], c=params3$c[k],
                     d=params3$d[k], e=params3$e[k]), col=k)
    lines(x=xs, y=b5(xs, b=params4$b[k], c=params4$c[k],
                     d=params4$d[k], e=params4$e[k], f=params4$f[k]),  col=k)
    for(j in 2:ncol(A)){
...
plot_together(Cycle, DF1, DF2, DF3, DF4)