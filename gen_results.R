...
gen_results <- function(subs, params){
  ...
  if(modparams[[1]]$fits[[1]]$MODEL$name %in% c("l5", "l4", "b5", "b4")){
    #finding CT value
  ...
  #LSTAR model
  else if(is.null(modparams[[1]]$fits[[1]]$MODEL$name) == TRUE){
    ...
    rss.mat <- matrix(c(unlist.rsslstar, unlist.rsslstarr, unlistcycCTthr), ncol=3)
  } #else if lstar
