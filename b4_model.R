...
subplot_b4 <- function(listdf){
  #get fitted models of each replicate within list
  par <- lapply(listdf, function(x) sub_genparams(b4, x)) #KW test results
  #plot the model
  for(i in 1:length(listdf)){
    xs = listdf[[i]]$Cycle
    tryCatch({plot(x=xs, y=b4_model(xs, b=par[[i]]$params$b[1], c=par[[i]]$params$c[1],
    ...
    for(k in 2:length(par[[i]]$params)){
      lines(x=xs, y=b4_model(xs, b=par[[i]]$params$b[k], c=par[[i]]$params$c[k],
